...
def weighted_choice(weights, exclude = None):
    r = random.Random()
    total_weight = 0
    rnd = 0
    pick = None
    ...
    if exclude:
        weights = dict((choice, weight) for (choice, weight) in weights.iteritems() if not choice in exclude)
        
    rnd = r.random() * total_weight
...
class Cultures:
    @staticmethod
    def generate(tl, used_cultures):
        culture = None
        
        if tl == 0 or tl == 1:
            culture = weighted_choice({
                    "Andean": 0.465,
                    "Hittite": 4.16,
                    "Chinese": 21.31,
                    "Egyptian": 24.09,
            ...
            if culture == "Retrogression":
                culture = Cultures.generate(2, used_cultures)
        elif tl == 2:
            culture = weighted_choice({
                    "Mississippian": 0.465,
                    "Andean": 0.696,
            ...
            if culture == "Survival":
                culture = Cultures.generate(1, used_cultures)
            ...
            if culture == "Survival":
                culture = Cultures.generate(4, used_cultures)
                
        elif tl == 7 or tl == 8 or tl == 9:
            culture = weighted_choice({
                    "Malay": 0.93,
                    "West African": 0.93,
...
class Unities:
    ...
    def generate(culture):
        ...
        if dieRoll == 2:
            ...
            return "Empire"
        elif dieRoll == 5 or dieRoll == 6:
            return "Empire with satellite states"
        elif dieRoll == 7:
            return "Empire with rivals"
        elif dieRoll == 8:
            return "Bipolar"
        elif dieRoll == 9 or dieRoll == 10 or dieRoll == 11:
            return "Multipolar"
        elif dieRoll == 12:
            return "Diffuse"
...
def modifyUnityRoll(unityRoll, culture):
    if culture in CIV_UNITY_MODIFIERS:
...
STATES = {
    ...
    "Chinese": {
        ...
        "Manchu": {
            ...
            "generate_official_name": lambda civ, structure:
                {
                "Anarchy": "Manchu",
                "Tribal": "Manchu",

                "Representative democracy": "Manchu Republic",
                "Athenian democracy": "Manchu Republic",
                }.get(structure, "Empire of the Great Qing"),
            "generate_capital": lambda civ, structure:
                "Peking",
            },
        "Taiping": {
            "generate_official_name": lambda civ, structure:
    ...
    "Japanese": {
        ...
        "Japan": {
            "generate_official_name": lambda civ, structure:
                {
                "Caste": "Japanese Shogunate",
                "Feudal": "Japanese Shogunate",
                }.get(structure, "Japan"),
            }
        },
    ...
    "Mesoamerican": {
        ...
        "Aztecs": {
            "generate_official_name": lambda civ, structure:
                {"Anarchy": "Mexica Peoples",
                 "Representative democracy": "United Mexican States"
                 }.get(structure, "Aztec Triple Alliance"),
            }
        },
    ...
    "Orthodox": {
        ...
        "Russia": {
            "generate_official_name": lambda civ, structure:
                ...
                "Empire with satellite states": "Russian Empire",
                "Empire with rivals": "Russian Empire",
                }.get(civ.unity, "Tsardom of Russia"),
            },
        "Soviet Union": {
            "generate_official_name": lambda civ, structure:
                {
                "Empire": "Union of Soviet Socialist Republics",
                "Empire with satellite states": "Union of Soviet Socialist Republics",
                "Empire with rivals": "Union of Soviet Socialist Republics",
                }.get(civ.unity, "Russian Soviet Federative Socialist Republic"),
            },
        "Russian Republic": {},
        "Ukraine": {}
        },
    ...
    "Steppe": {
        ...
        "Mongols": {
            ...
            "generate_official_name": lambda civ, structure:
                {
                "Anarchy": "Mongol Tribes",
                "Tribal": "Mongol Tribes",
                
                "Representative democracy": "Republic of Mongolia",
                "Athenian democracy": "Republic of Mongolia",
                }.get(structure, "Mongol Khaganate"),
            "Golden Horde": {},
            "Timurid Empire": {}
            },
        },
...
class StateNames:
    @staticmethod
    def generate(culture, tl, structure, used_names):
        state_choices = STATES[culture]
        name_weights = dict((name, props["weight_by_tl"][tl])
                            if "weight_by_tl" in props and tl in props["weight_by_tl"] else
                            (name, 1)
                            for (name,props) in state_choices.iteritems())
        
        return weighted_choice(name_weights, used_names)

STATE_POLITICAL_STRUCTURES = {
    "Anarchy": 0.465,
